...
const migrationSql = readFileSync(join(__dirname, '../migrations/0006_institution_portal_accounts.sql'), 'utf8');
...
const now = Date.now();
try {
  await sql`INSERT INTO drizzle.__drizzle_migrations (hash, created_at) VALUES ('0006_institution_portal_accounts', ${now})`;
